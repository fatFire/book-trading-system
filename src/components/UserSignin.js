...
export default function UserSignin() {
  ...
  const mutationFn = async data => {
    try {
      const user = await axios.post('http://localhost:1337/auth/local', data);
      const cart = await axios.get(
        `http://localhost:1337/carts?users_permissions_user=${user.data.user.id}`,
        {
          headers: {
      ...
      console.log(user);
      return { user: user.data, cart: cart.data[0] };
    } catch (e) {
      throw e;
    }
  };
